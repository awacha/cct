...
class Transmission(Script):
    ...
    script = """
        ...
        @startloop
            ...
            exposemulti(exptime, nimages, _config['path']['prefixes']['tra'], 0.02, (currentsample, 'dark', nimages))
            print('Exposing empty beam')
            sample(emptyname)
            shutter('open')
            exposemulti(exptime, nimages, _config['path']['prefixes']['tra'], 0.02, (currentsample, 'empty', nimages))
            shutter('close')
            print('Exposing sample')
            sample(currentsample)
            shutter('open')
            exposemulti(exptime, nimages, _config['path']['prefixes']['tra'], 0.02, (currentsample, 'sample', nimages))
            shutter('close')
