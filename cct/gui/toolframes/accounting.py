...
class AccountingFrame(ToolFrame):
    def _init_gui(self, *args):
        ...
        if sel.get_active() is None:
            sel.set_active(0)
        self._instrument.accounting.connect('project-changed', self.on_project_changed)
        self.on_project_changed(self._instrument.accounting)

    def on_projectid_changed(self, comboboxtext):
        if hasattr(self, '_projectid_changed_disable'):
            return
        pid = comboboxtext.get_active_text()
        if self._instrument.accounting.get_project().projectid != pid:
            self._instrument.accounting.select_project(pid)
    ...
    def on_project_changed(self, accounting):
        ...
        pidsel.remove_all()
        for i, project in enumerate(self._instrument.accounting.get_projectids()):
            pidsel.append_text(project)
            if project == self._instrument.accounting.get_project().projectid:
                pidsel.set_active(i)
        self._builder.get_object('proposer_label').set_text(self._instrument.accounting.get_project().proposer)
        self._builder.get_object('projectname_label').set_text(self._instrument.accounting.get_project().projectname)
