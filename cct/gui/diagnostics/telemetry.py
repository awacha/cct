...
class ResourceUsage(ToolWindow):
    ...
    def update_telemetry(self, label, tm):
        ...
        model.clear()
        model.append(['Process name:', tm.processname])
        ...
        model.append(['Input queue length:', '{}'.format(tm.inqueuelen)])
        ...
        model.append(['Total physical memory (GB):', '{:.2f}'.format(tm.totalphysmem / 1024 / 1024 / 1024)])
        model.append(['Free physical memory (GB):', '{:.2f}'.format(tm.freephysmem / 1024 / 1024 / 1024)])
        model.append(['Total swap space (GB):', '{:.2f}'.format(tm.totalswap / 1024 / 1024 / 1024)])
        model.append(['Free swap space (GB):', '{:.2f}'.format(tm.freeswap / 1024 / 1024 / 1024)])
        model.append(['Load averages:', '{}'.format(tm.loadavg)])
