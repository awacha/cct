...
class GeometryOptimizer(QtCore.QObject):
    ...
    def start(self, maxsamplediameter: float, qmin: float, l1min: float, l2min: float, lmax: float):
        ...
        self.queue = Queue()
        self.process = Process(target=self._worker, name='optimization worker',
                               args=(self.stopevent, self.queue, self.config, maxsamplediameter, qmin, l1min, l2min, lmax))
    ...
    def _worker(self, stopevent: Event, outqueue: Queue, config: Config, maxsamplediameter: float, qmin: float, l1min: float = 0.0,
                l2min: float = 0.0,
                lmax: float = 0.0):
        ...
        for ph1, ph2, ph3, bs, (l1_elementcount, l2_elementcount, spacers), (
            ...
            self.yieldspacers(config['geometry']['choices']['spacers']),
            self.yieldflightpipes(config['geometry']['choices']['flightpipes'])):
            if stopevent.is_set():
                break
