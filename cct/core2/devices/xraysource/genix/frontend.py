...
class GeniX(DeviceFrontend):
    ...
    def standby(self):
        if self.get('__status__') in [GeniXBackend.Status.off, GeniXBackend.Status.full, GeniXBackend.Status.unknown, GeniXBackend.Status.standby]:
            self.issueCommand('standby')
        else:
            raise self.DeviceError(f'Cannot set X-ray source to standby mode from mode {self.get("__status__")}')

    def rampup(self):
        if self.get('__status__') not in [GeniXBackend.Status.standby, GeniXBackend.Status.full]:
            raise self.DeviceError(f'Cannot put X-ray source to full power mode from mode {self.get("__status__")}')
    ...
    def xraysOff(self):
        if (self.get('ht') > 0) or (self.get('current') > 0):
    ...
    def onCommandResult(self, success: bool, commandname: str, result: str):
        ...
        if commandname == 'shutter' and not success:
            self._logger.error(f'Cannot {"close" if self.get("shutter") else "open"} shutter')
            self.shutter.emit(self.get('shutter'))
    ...
    def toNeXus(self, grp: h5py.Group) -> h5py.Group:
        ...
        self.create_hdf5_dataset(grp, 'power', self.get('power'), units='W')
        self.create_hdf5_dataset(grp, 'energy', self.get('ht'), units='keV')
        self.create_hdf5_dataset(grp, 'current', self.get('current'), units='mA')
        self.create_hdf5_dataset(grp, 'voltage', self.get('ht'), units='kV')
        return grp
