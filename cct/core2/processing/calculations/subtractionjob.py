...
class SubtractionJob(BackgroundProcess):
    ...
    def __init__(self, jobid: int, h5file: str, h5lock: multiprocessing.Lock, stopEvent: multiprocessing.Event,
        ...
        if subtractedname is None:
            self.subtractedname = f'{samplename}-{backgroundname if backgroundname is not None else "constant"}'
        else:
            self.subtractedname = subtractedname
    ...
    def subtractconstant(self, distkey: str) -> Tuple[Exposure, Curve, Curve]:
        ...
        if self.scalingmode == SubtractionScalingMode.PowerLaw:
            # fit a power-law
            curve = exposure.radial_average(np.linspace(self.interval[0], self.interval[1], self.interval[2])).sanitize()
            ...
            bglevel = (0.0, 0.0)
        ex = Exposure(exposure.intensity - bglevel[0], exposure.header, (exposure.uncertainty**2 + bglevel[1]**2)**0.5, exposure.mask)
        ...
        if self.scalingmode == SubtractionScalingMode.Unscaled:
            ...
            if self.scalingmode == SubtractionScalingMode.Interval:
                ...
                def powerlaw_goodnessoffit(paramarray):
                    odrdata = scipy.odr.RealData(rad.q, rad.intensity-bgrad.intensity*paramarray[0], rad.quncertainty, (rad.uncertainty**2 + paramarray[0]**2 * bgrad.uncertainty**2)**0.5)
            ...
            assert False
        ex = Exposure(exposure.intensity - factor[0]*bg.intensity, exposure.header, (exposure.uncertainty**2 + factor[0]**2*bg.uncertainty**2 + bg.intensity**2*factor[1]**2)**0.5)
    ...
    def main(self):
        ...
        for dk in distkeys_sample:
            ...
            with self.h5io.writer('Samples') as grp:
                sub.header.title = self.subtractedname
