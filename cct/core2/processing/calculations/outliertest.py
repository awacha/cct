...
class OutlierTest:
    ...
    def _polynomialFtest(self, deg):
        ...
        paramconst = modelconst.make_params(c=self.score.mean())
        parampoly = modelpoly.make_params(c0=self.score.mean(), **{f"c{i}":0.0 for i in range(1, deg+1)})
        resultpoly = modelpoly.fit(data=self.score, x=xfit, params=parampoly)
        resultconst = modelconst.fit(data=self.score, x=xfit, params=paramconst)
