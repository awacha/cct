...
class AnisotropyEvaluator(QtWidgets.QWidget, Ui_Form, ToolWindow):
    ...
    def onQRangeSelected(self, qmin: float, qmax: float):
        ...
        ex = self.exposure()
        ex.mask_nonfinite()
        phi, intensity, error, area, mask = azimint(ex.intensity, ex.error, ex.header.wavelength.val,
                                                    ex.header.distance.val,
                                                    ex.header.pixelsizex.val, ex.header.beamcentery.val,
                                                    ex.header.beamcenterx.val,
                                                    (ex.mask == 0).astype(np.uint8), qmin=min(qmin, qmax),
                                                    qmax=max(qmin, qmax),
                                                    returnmask=True
                                                    )
        phi[area == 0] = np.nan
