...
class MotorAutoCalibrationModel(QtCore.QAbstractItemModel):
    ...
    def addMotor(self, name: str):
        ...
        self._rows.append(
            [name, mot.where(), mot.get_variable('softleft'), mot.get_variable('softright'), mot.where(), None, None])
        self.endInsertRows()
    ...
    def headerData(self, section: int, orientation: QtCore.Qt.Orientation, role: int = ...):
        if orientation == QtCore.Qt.Horizontal and role == QtCore.Qt.DisplayRole:
            return ['Motor name', 'Position', 'Soft left', 'Soft right', 'Position before', 'Position after', 'Delta'][
                section]
        else:
            return None
