...
class SampleScalerModel(QtCore.QAbstractItemModel):
    ...
    def setData(self, index: QtCore.QModelIndex, value: typing.Any, role: int = ...):
        if index.column()==1 and role == QtCore.Qt.EditRole:
            ...
            self.dataChanged.emit(self.index(index.row(), index.column()), self.index(index.row(), index.column()), [QtCore.Qt.DisplayRole])
            return True
        elif index.column()==0 and role==QtCore.Qt.CheckStateRole:
            self._selected[self._samples[index.row()]]=value
            self.dataChanged.emit(self.index(index.row(), index.column()), self.index(index.row(), index.column()), [QtCore.Qt.DisplayRole, QtCore.Qt.CheckStateRole])
        return False
