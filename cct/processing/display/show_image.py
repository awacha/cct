...
def pixel_to_q(row: float, column: float, bcrow: float, bccol: float, pixelsizerow: float, pixelsizecol: float,
               distance: float, wavelength: float) -> Tuple[float, float]:
...
def show_scattering_image(fig: Figure, group: Group, showmask: bool = True, showcenter: bool = True):
    ...
    ymin, xmin = pixel_to_q(0, 0, group.attrs['beamcentery'], group.attrs['beamcenterx'], group.attrs['pixelsizey'],
                            group.attrs['pixelsizex'], group.attrs['distance'], group.attrs['wavelength'])
    ymax, xmax = pixel_to_q(img.shape[0], img.shape[1], group.attrs['beamcentery'], group.attrs['beamcenterx'],
                            group.attrs['pixelsizey'], group.attrs['pixelsizex'], group.attrs['distance'],
                            group.attrs['wavelength'])

    ret = ax.imshow(img, extent=[xmin, xmax, -ymax, -ymin], aspect='equal', origin='upper', interpolation='nearest',
                    norm=LogNorm())
    if showmask:
        ...
        if (mask == 0).sum():  # there are some masked pixels
            ...
            mf[mask != 0] = np.nan
            ax.imshow(mf, extent=[xmin, xmax, -ymax, -ymin], aspect='equal', origin='upper', interpolation='nearest',
                      alpha=0.8, norm=Normalize(), cmap=matplotlib.cm.gray_r)
    if showcenter:
