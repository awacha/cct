...
class ExpTimeModel(QtCore.QAbstractItemModel):
    ...
    def __init__(self, parent, group:h5py.Group):
        ...
        for sn in sorted(group.keys()):
            for dist in sorted(group[sn].keys(), key=float):
                self._data.append((sn, float(dist), group[sn][dist].attrs['exposuretime'], len(group[sn][dist]['curves'])))
    ...
    def headerData(self, section: int, orientation: QtCore.Qt.Orientation, role: int = ...):
        if orientation==QtCore.Qt.Horizontal and role==QtCore.Qt.DisplayRole:
            return ['Sample', 'Distance (cm)', 'Live time (h)', 'Rel. live time', '# of exposures', 'Frame time'][section]
        return None
